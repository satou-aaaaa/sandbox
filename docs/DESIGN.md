...
| completedConstructionCost | CompletedConstructionCostInput（任意） | 完成工事原価報告書の内訳（M10・様式第十六号の一部用に追加。§4.11参照） |
...
### 4.11 CompletedConstructionCostInput（完成工事原価報告書の内訳。M10）

完成工事原価報告書（様式第十六号の一部）は、材料費・労務費・外注費・経費の
4区分・6項目に固定されており、勘定科目の追加は認められていない
（ADR-0010参照）。
...
| subcontractedLaborCost | number（任意） | （うち）労務外注費。労務費の内訳 |
| subcontractCost | number | 外注費 |
| expenses | number | 経費 |
| personnelExpenses | number（任意） | （うち）人件費。経費の内訳 |

`ApplicantProfile.completedConstructionCost`（§4.1）として保持する。
合計額（完成工事原価）は損益計算書の完成工事原価と一致する必要があるが、
損益計算書自体は対象外（ADR-0010）のため、整合性チェックは実装せず
注記での案内に留める（§5.19参照）。
...
- `/submit` はセッションごとに `crypto.randomUUID()` でディレクトリを分け、
  7様式すべてのdocxを `outDir/<sessionId>/` に生成する。この動作は
...
**標準の一括生成（`/submit`）には含めない**: 経審は新規許可申請とは
別の手続き（既存の許可を前提に、公共工事入札参加のために別途申請するもの）
であり、すべての新規申請者が必要とするわけではない。他の様式（様式第一号・
第二号・第六号・第七号・第八号・第十六号の一部・第二十号の二の7様式。
様式第十六号の一部はM10で追加）は建設業許可の新規申請一式に
...
### 5.19 `src/documents/youshiki16.js` — 完成工事原価報告書（M10・実装済み）

貸借対照表・損益計算書本体は対象外とし（ADR-0010）、様式第十六号のうち
**完成工事原価報告書の部分のみ**をdocxで生成する。材料費・労務費（うち
労務外注費）・外注費・経費（うち人件費）の4区分・6項目に固定されており、
この構成に勘定科目を追加しない（複数の情報源で「勘定科目の追加は
認められていない」ことを確認済み。§4.7・§4.11参照）。

**標準の一括生成（`/submit`）に含める**（既存の6様式に追加し7様式になる）:
完成工事原価報告書を含む財務諸表は、経審だけでなく既存の許可を持つ会社が
新規に別業種の許可を申請する場合にも添付書類として必要になりうる書類であり
（`youshiki25-14.js`（経審専用手続き）とは異なり）、経審対応の有無に
関わらず必要になりうる。役員・営業所・工事経歴と同様に、Webフォーム
（`src/web/formPage.js`）にも入力セクションを追加済み（M8で工事経歴の
フォーム連携が漏れていた反省を踏まえ、実装と同じPRでセットで対応した）。
必須の4項目（材料費・労務費・外注費・経費）は既定値0の数値入力、
任意の内訳2項目（労務外注費・人件費）は未入力を「0円」と区別するため、
空欄のままなら `undefined` として送信する（他の必須数値項目とは
異なる挙動である点に注意。§5.10のブラウザ側JS実装コメント参照）。

- `resolveYoushiki16Rows(profile)`: `profile.completedConstructionCost`
  （§4.11）から6項目のラベル・値のペアを組み立てる純粋関数。合計額
  （完成工事原価 = 材料費+労務費+外注費+経費）も算出して表示する
- 完成工事原価の合計額は損益計算書の完成工事原価と一致する必要があるが、
  損益計算書自体は対象外のため、整合性チェックは実装せず「損益計算書の
  完成工事原価と一致させること」という注記に留める（ADR-0010）
- 消費税の税込・税抜は変換しない（`youshiki2.js` と同じ方針。経審提出時は
  税抜金額が必須だが、税率を仮定した自動換算はしない）
- `profile.completedConstructionCost` が未入力の場合はその旨の1行を返す
  （`youshiki2.js` の工事経歴0件時と同じパターン）
- 他の様式モジュールと同じ `buildYoushiki16Document(profile)` /
  `writeYoushiki16Docx(profile, outPath)` の2関数構成、`common.js` の
  `buildLabeledTable`（ラベル・値の2列で足りるため`buildHeaderedTable`は
  使わない）・`buildDisclaimerParagraph` を踏襲する
...
- 例外として、`src/web/formPage.js` のフォーム画面が生成する `<script>` 内の
  ...
  過去に「工事経歴がbuildProfile()で収集されていない」というバグが
  ブラウザでの手動確認でしか発覚しなかった経緯（PR #25）を踏まえた例外対応で、
  コードをテスト用に再実装せず本番と同じ文字列をそのまま実行することで
...
### 7.1 ミューテーションテスト（Stryker Mutator。2026年9月導入）

行・分岐カバレッジ（`npm run test:coverage`）は「そのコードが実行されたか」
しか示さず、実行されていてもテストのassertが緩ければバグを見逃す（例:
`>=` を `>` に書き換えても既存テストが気づかない等）。この弱点を補うため、
[Stryker Mutator](https://stryker-mutator.io/)によるミューテーションテストを
導入した（`stryker.config.mjs`）。

- **実行方法**: `npm run test:mutation`（`stryker run`のエイリアス）。
  ミュータント1件ごとに`node --test`（全テスト）を再実行する
  「commandランナー」を使うため、日常のPRごとに毎回実行するものでは
  なく、大きな変更の節目やテスト方針の見直し時に手動実行する運用とする
  （実行には数分〜数十分かかる。詳細な設計判断は`stryker.config.mjs`の
  コメントを参照）。
- **対象範囲**: 要件判定・欠格事由判定・日付/金額計算・CSV相互変換など
  「間違えると実害が大きい」ロジックに絞る（`src/documents/**`・
  `src/web/**`等の書類生成・画面表示は対象外。理由は設定ファイルの
  コメントを参照）。
- **既知の非互換**: `typescript@7`系では`ts.parseConfigFileTextToJson`が
  削除されており、Stryker（9系・10系とも）が内部でこのAPIを呼ぶため
  tsconfig.jsonが存在するプロジェクトでは即座にクラッシュする既知の問題が
  ある。本プロジェクトはStrykerのtypescript-checkerプラグインを使わない
  ため、`tsconfigFile`に存在しないパスを指定してこの前処理自体をスキップする
  回避策を`stryker.config.mjs`に適用している。
- **導入時のベースライン**: 2026年9月時点のミューテーションスコアは、
  対象範囲（要件判定・欠格事由判定・日付/金額計算・CSV相互変換。
...
日付計算・CSV相互変換のような「入力の組み合わせが実質無限にある」純粋関数は、
具体例ベースのテストだけでは書き手が思いついた入力しか検証できない。
[fast-check](https://fast-check.dev/)を使い、ランダムな入力を大量に生成して
「入力によらず常に成り立つべき性質」を検証するテストを、対象を絞って
追加した（乱用すると通常のテストより読みにくくなるため、性質を明確に
言語化できる箇所に限定している）:

- `test/dateUtils.test.js`: `daysUntil`の反対称性・反射性・実カレンダーとの
  整合性（n日後との差が常にnになること）
- `test/clientCsv.test.js`: `clientsToCsv`→`clientsFromCsv`の往復性
...
- **M9 経営規模等評価申請書・総合評定値請求書対応**: 実装済み。様式第二十五号の
  ...
  個別生成（`npm run gen:youshiki25-14`）のみとした。
- **M10 財務諸表対応**: 実装済み（一部）。完成工事原価報告書（様式第十六号の
  一部。材料費・労務費・外注費・経費の4区分・6項目に固定）のみ対応した
  （§4.11・§5.19）。貸借対照表（様式十五号）・損益計算書本体（様式十六号）・
  株主資本等変動計算書（様式十七号）・注記表（様式十七号の二）は、法定の
  勘定科目分類（国土交通大臣告示）の全体を一次資料で確認できなかったため
  対象外とした（FR-7.3・ADR-0010参照）。完成工事原価報告書は7様式一括生成
  （`/submit`）に含め、Webフォームにも入力セクションを追加した。
