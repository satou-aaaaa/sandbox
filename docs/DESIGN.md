...
### 5.17 `src/documents/youshiki2.js` — 工事経歴書（M8・経審書類準備支援の第1弾）
...
- **M8 経営事項審査（経審）書類準備支援**: 設計完了・実装未着手。
  フェーズ1は工事経歴書（様式第二号）のdocxサマリー生成のみ（§4.9・§5.17）。
  経審の評点計算（X1・X2・Y・Z・W・総合評定値P）、経営規模等評価申請書・
  経営状況分析申請書の生成、財務諸表（様式第十五号〜十七号）のサマリー生成は
  対象外とした（国が定期改定する換算表・法令上民間機関が算出するY点を扱う
  専門領域であるため。ADR-0009参照）。財務諸表対応等を将来検討する場合は
  M9以降の別フェーズとする。詳細は `docs/REQUIREMENTS.md` §4.7を参照。
