...
- **特定技能2号への拡張**: 対象分野が限定的（本書執筆時点では建設・
  造船等の一部分野）で、在留期間の更新に上限がなく家族帯同も可能という
  1号との違いが大きい。`fieldRegistry.js`に「2号への移行が可能な分野か」
  のフラグを追加し、`tokuteiGinouSchedule.js`の通算5年上限警告に
  「2号への移行検討」の案内を含める形で、既存の型・レジストリ構造を
  大きく変えずに拡張できる見込みが高い（本モジュールの分野レジストリ
  設計が2号対応の土台になることを想定している）
