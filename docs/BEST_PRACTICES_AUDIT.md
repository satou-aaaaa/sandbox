...
Googleのeng-practicesが挙げる8つのレビュー観点（設計・機能性・複雑性・
テスト・命名・コメント・スタイル・ドキュメント）は、他者レビューが
存在しない単独開発でも「マージ前の自己チェックリスト」として有効。

| 項目 | 状態 | 補足 |
|---|---|---|
| セルフレビューチェックリスト | ✅ | `.github/pull_request_template.md` にGoogle eng-practicesの8項目を追加。PR作成時に必ず目にする位置に配置 |
